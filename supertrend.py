...
# TODO Could create a list of Symbols to fetch and loop through
def fetch_data(symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
    """Fetch symbol data from binance exchange and output dataframe"""
    # 1. Grab our data
...
def find_buy_sell_timestamps(data: pd.DataFrame):
...
def supertrend(data: pd.DataFrame, period: int = 7, multiplier: int = 3):
...
symbols = st.multiselect("Select the symbol(s): ", available_symbols)
...
for symbol in symbols:
    ...
    df = fetch_data(symbol=symbol, timeframe="1d", limit=100)

    # 3. Perform supertrend calculation
    supertrend_df = supertrend(df)

    # 4. Display results
    st.dataframe(supertrend_df)
